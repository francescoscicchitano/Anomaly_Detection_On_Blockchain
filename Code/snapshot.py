...
class SnapshotCallbackBuilder:
    ...
    def get_callbacks(self, model_prefix='Model'):
        ...
        if not os.path.exists('weights_synthetic/'):
            os.makedirs('weights_synthetic/')
        
        if not os.path.exists('logs/'):
            os.makedirs('logs/')
        
        if not os.path.exists('logs_synthetic/'):
            os.makedirs('logs_synthetic/')
        
        callback_list = [callbacks.ModelCheckpoint("weights/%s-Best.h5" % model_prefix, monitor="val_loss",
                                                    save_best_only=True, save_weights_only=True, mode = 'min'),
                         callbacks.CSVLogger('logs/log-%s-Best.csv' % model_prefix),
                         callbacks.LearningRateScheduler(schedule=self._cosine_anneal_schedule),
                         SnapshotModelCheckpoint(self.T, self.M, fn_prefix='weights/%s' % model_prefix)]
        '''     
        
        callback_list = [callbacks.ModelCheckpoint("weights_synthetic/%s-Best.h5" % model_prefix, monitor="val_loss",
                                                    save_best_only=True, save_weights_only=True, mode = 'min'),
                         callbacks.CSVLogger('logs_synthetic/log-%s-Best.csv' % model_prefix),
                         callbacks.LearningRateScheduler(schedule=self._cosine_anneal_schedule),
                         SnapshotModelCheckpoint(self.T, self.M, fn_prefix='weights_synthetic/%s' % model_prefix)]
        '''
